...
public class Utils {
	...
	public static double averavge(ArrayList<Double> diff, ArrayList<Double> diff_sample_weight) {
		// TODO Auto-generated method stub
		double sum = 0, sumWeight = 0;
		for (int i = 0; i < diff.size(); i++) {
			sum += diff.get(i) * diff_sample_weight.get(i);
			sumWeight += diff_sample_weight.get(i);
		}
		return sum / sumWeight;
	}

	public static double averavge(double[] y, double[] sample_weight) {
		double sum = 0, sumWeight = 0;
		for (int i = 0; i < y.length; i++) {
			sum += y[i] * sample_weight[i];
			sumWeight += sample_weight[i];
		}
		return sum / sumWeight;
	}
}