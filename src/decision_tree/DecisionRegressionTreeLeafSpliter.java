...
public class DecisionRegressionTreeLeafSpliter extends DecisionTree {
	...
	public DecisionRegressionTreeLeafSpliter(int num_leafs, int random_state, double sample_rate, double feature_rate,
			double min_leaf_sample, LabeledSample[][] preSortedSampleArrays) {
	...
	private QueueNode createTree(int l, int r) {
		Node root = new Node();
		...
		qNode.splitResult = result;
		root.split_feature = result.best_feature;
		root.split_val = result.best_split_val;
		qNode.gain = calCurrentLoss(l, r) - result.split_error;
		return qNode;
	}

	private int doSplit(QueueNode node, int l, int r) {
		SplitResult result = node.splitResult;
		if (result == null)
			return 0;
	...
	private double calCurrentLoss(int l, int r) {
		double sum = 0;
		double sumWeight = 0;
		for (int i = l; i <= r; i++) {
			sum += sampleSortedByFeatureArrays[0][i].y;
			sumWeight += sampleSortedByFeatureArrays[0][i].weight;
		}
		double avg = sum / sumWeight;
		double error = 0;
		for (int i = l; i <= r; i++) {
			error += (sampleSortedByFeatureArrays[0][i].y - avg) * (sampleSortedByFeatureArrays[0][i].y - avg)
					* sampleSortedByFeatureArrays[0][i].weight;
		}
		return error;
	}
	...
	private Comparator<QueueNode> cmp = new Comparator<QueueNode>() {
		public int compare(QueueNode e1, QueueNode e2) {
			return Double.valueOf(e2.gain).compareTo(e1.gain);
		}
	};

	public void fit(double[][] X, double[] Y, double[] sample_weight) {
		random_state = random.nextInt();
		Queue<QueueNode> queue = new PriorityBlockingQueue<>(this.num_leafs * 2, cmp);
		QueueNode qNode = createTree(0, X.length - 1);
		this.root = qNode.node;
		queue.add(qNode);
		int currentLeaf = 1;
		while (!queue.isEmpty()) {
			qNode = queue.poll();
			System.out.println(qNode.gain);
			if (qNode.l == qNode.r)
				continue;
			int leftSize = doSplit(qNode, qNode.l, qNode.r);
			QueueNode qNodeLeft = createTree(qNode.l, qNode.l + leftSize - 1);
			QueueNode qNodeRight = createTree(qNode.l + leftSize, qNode.r);
			queue.add(qNodeLeft);
			queue.add(qNodeRight);
			qNode.node.leftNode = qNodeLeft.node;
			qNode.node.rightNode = qNodeRight.node;
			currentLeaf += 1;
			if (currentLeaf == this.num_leafs)
				break;
		}
	}
